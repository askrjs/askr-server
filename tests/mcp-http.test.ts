...
function application(
  ...
  mcp = createMcpServer({ name: "http", version: "1" }),
) {
...
describe("MCP Streamable HTTP", () => {
  it("should enforce negotiation, request limits, Origin, and Host", async () => {
    ...
    expect((await app.fetch(post(initialize, { accept: "application/json" }))).status).toBe(406);
    expect(
      (
        await app.fetch(
          post(initialize, {
  ...
  it("should create sessions only after successful initialization and bound capacity", async () => {
    ...
    const invalid = await app.fetch(
      post({ ...initialize, params: { ...initialize.params, protocolVersion: undefined } }),
    );
  ...
  it("should expire default-store sessions after the configured TTL", async () => {
    let now = 1_000;
    const mcp = createMcpServer({ name: "http", version: "1" });
    const terminate = vi.spyOn(mcp, "terminateSession");
    const app = application(
      true,
      {
        sessionTtlMs: 100,
