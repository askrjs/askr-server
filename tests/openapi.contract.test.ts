...
describe("OpenAPI public contract", () => {
  it("shares route registration between documentation and dependency-injected runtime", async () => {
    ...
    const handler = vi.fn(async (ctx, deps: typeof dependencies) => ctx.ok(await deps.users.find(ctx.params.id)));
  ...
  it("inherits groups and documents parameters, bodies, examples, and operation metadata", () => {
    ...
    const group = api.group("/v1/{tenant}").tags("Items").use(middleware)
      .pathParam("tenant", schema.string())
      .queryParam("locale", schema.string(), { example: "en" });

    group.post("/items", (ctx) => ctx.created())
      .operationId("createItem").summary("Create item").description("Creates one item")
      .headerParam("x-trace-id", schema.uuid()).cookieParam("sid", schema.string())
      .jsonBody(Input, { required: true, examples: { sample: { value: { title: "Hi" } } } })
      .formBody(Input).multipartBody(Input)
      .created(schema.object({ id: schema.integer({ minimum: 1 }) }), {
        examples: { result: { value: { id: 1 } } },
      })
      .deprecated().externalDocs("https://example.test/docs", "More docs");

    const operation = (api.toOpenApiDocument().paths as Record<string, Record<string, unknown>>)["/v1/{tenant}" + "/items"].post as Record<string, unknown>;
  ...
  it("aligns access with security and adds overridable Problem responses", async () => {
    ...
    group.get("/me", (ctx) => ctx.ok()).operationId("getMe").summary("Get me").ok()
      .unauthorized(schema.string(), { mediaType: "text/plain", description: "Custom challenge" });
    api.get("/signup", (ctx) => ctx.ok()).operationId("signup").summary("Sign up")
      .access(requireAnonymous(), security.none()).ok();
    ...
    const response = await createServerApp(api.createRouter()).fetch(new Request("http://example.test/secure/me"));
  ...
  it("supports schema composition, constraints, records, literals, and raw schemas", () => {
    ...
    const Base = api.schema("Base", schema.object({ id: schema.uuid(), active: schema.boolean({ default: true }) }));
    const Model = api.schema("Model", schema.allOf(
      Base,
      schema.object({
    ...
    api.get("/model", (ctx) => ctx.ok()).operationId("getModel").summary("Get model").ok(Model);
  ...
  it("supports named redirects, remaining server errors, and status ranges", () => {
    ...
    api.get("/responses", (ctx) => ctx.noContent()).operationId("responses").summary("Responses")
      .movedPermanently().found().seeOther().temporaryRedirect().permanentRedirect()
      .methodNotAllowed().notImplemented().response("4XX", undefined, { description: "Other client error" });
    const responses = (api.toOpenApiDocument().paths as Record<string, any>)["/responses"].get.responses;
    expect(Object.keys(responses)).toEqual(["301", "302", "303", "307", "308", "405", "501", "4XX"]);
  });
});
