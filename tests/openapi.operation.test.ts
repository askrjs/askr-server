...
describe("OpenAPI executable operations", () => {
  it("should read declared sources independently given colliding field names", async () => {
    ...
    expect(
      operation.parameters?.map(({ name, in: location, schema: projection }) => ({
        name,
        location,
        projection,
      })),
    ...
    const response = await createServerApp(api.createRouter()).fetch(
      new Request("http://example.test/items/path?id=query&tag=one&tag=two", {
        method: "POST",
  ...
  it("should not consume an undeclared body before the handler", async () => {
    ...
    finish(route);
    const response = await createServerApp(api.createRouter()).fetch(
      new Request("http://example.test/items?mode=query", {
        method: "POST",
        headers: { "content-type": "application/json" },
  ...
  it("should reject a second schema declaration for executable operation input", () => {
    ...
    finish(route);
    (route as unknown as { queryParam(name: string, value: unknown): void }).queryParam(
      "page",
      schema.string(),
    );
