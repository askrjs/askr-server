...
describe("model binding source edges", () => {
  it("preserves empty, bare, encoded, null-byte, and case-sensitive query keys", async () => {
    const response = await echoApp("GET").fetch(new Request(
  ...
  it("preserves three or more repeated query values in insertion order", async () => {
    const response = await echoApp("GET").fetch(new Request(
      "http://example.test/items/1?tag=first&tag=&tag=third&tag=fourth",
    ));
  ...
  it("replaces an entire body value at each later source boundary", async () => {
    const response = await echoApp("POST", "/items/{value}").fetch(new Request(
      "http://example.test/items/route?value=query-one&value=query-two&body-only=yes",
      {
        method: "POST",
  ...
  it("excludes every request header while leaving explicit access available", async () => {
    ...
    const result = await (await app.fetch(new Request("http://example.test/?authorization=query", {
      headers: {
  ...
  it("returns an ordinary plain object even though collection dictionaries are prototype-free", async () => {
    ...
    await expect((await app.fetch(new Request("http://example.test/1?q=yes"))).json()).resolves.toEqual({
  ...
  it("binds query and route values on GET without attempting body parsing", async () => {
    ...
    await expect(bind({ request, params: { id: "1" }, url, query: url.searchParams }))
...
describe("model binding cache edges", () => {
    ...
    const app = createServerApp({
      routes: [{
        ...
        handler: async (ctx) => {
          ...
          return ctx.json({ samePromise: firstPromise === secondPromise, sameObject: first === second });
        },
      }],
    });
    await expect((await app.fetch(new Request("http://example.test/", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: "{}",
    }))).json()).resolves.toEqual({ samePromise: true, sameObject: true });
  });
  ...
  it("shares the cached object between route middleware and the handler", async () => {
    ...
    await expect((await app.fetch(new Request("http://example.test/?q=yes", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: "{\"body\":true}",
    }))).json()).resolves.toEqual({ identical: true, value: { body: true, q: "yes" } });
  });
  ...
  it("preserves mutations because repeated calls return the same model object", async () => {
    ...
    await expect((await app.fetch(new Request("http://example.test/?q=yes"))).json())
  ...
  it("caches a failed binding promise and rejection object", async () => {
      ...
      routes: [{
        ...
        handler: async (ctx) => {
          ...
          return ctx.json({
            ...
            sameReason: left.status === "rejected" && right.status === "rejected" && left.reason === right.reason,
          });
        },
      }],
    });
    await expect((await app.fetch(new Request("http://example.test/", {
      ...
      headers: { "content-type": "application/json" },
      body: "{",
    }))).json()).resolves.toEqual({ samePromise: true, sameReason: true });
  });
});
