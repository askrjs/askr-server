...
describe("CORS middleware", () => {
  ...
  it.each([
    ...
    const response = await appWith(middleware).fetch(
      new Request("http://example.test/", {
  ...
  it("should route dynamic wildcard credential failures through onError", async () => {
    ...
    const response = await app.fetch(
      new Request("http://example.test/", { headers: { origin: "https://app.test" } }),
    );
  ...
  it("should support dynamic origin rejection and case-insensitive Vary deduplication", async () => {
    const app = createServerApp({
      ...
      routes: [
        { path: "/", handler: () => text("ok", { headers: { vary: "origin, ACCEPT-ENCODING" } }) },
      ],
    });
    ...
    expect(
      (
        await app.fetch(
          new Request("http://example.test/", { headers: { origin: "https://app.invalid" } }),
        )
      ).status,
    ).toBe(403);
  });
