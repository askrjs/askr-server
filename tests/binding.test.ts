...
function bindingApp(path = "/items/{id}") {
  return createServerApp({
    routes: [{ path, method: ["POST", "DELETE"], handler: async (ctx) => ctx.json(await ctx.bind()) }],
  });
}
...
describe("model binding", () => {
  ...
  it("should parse standard and structured-suffix JSON objects", async () => {
    ...
    for (const contentType of ["application/json; charset=utf-8", "application/merge-patch+json"]) {
      ...
      await expect(response.json()).resolves.toMatchObject({ nested: { active: true }, values: [1, 2] });
    }
  });

  it("should preserve repeated query and URL-encoded form values as arrays", async () => {
    const response = await bindingApp().fetch(new Request(
      "http://example.test/items/1?tag=query-one&tag=query-two",
      {
        method: "POST",
  ...
  it("should parse multipart text fields and files while preserving repetition", async () => {
    ...
    await expect((await app.fetch(new Request("http://example.test/upload", {
      method: "POST",
      body: form,
  ...
  it("should bind bodies on DELETE and other body-capable methods", async () => {
    const response = await bindingApp().fetch(new Request("http://example.test/items/1", {
      method: "DELETE",
  ...
  it("should ignore unsupported body media types without consuming the body", async () => {
    ...
    const result = await (await app.fetch(new Request("http://example.test/?query=yes", {
      method: "POST",
  ...
  it("should return the same bound object and read a supported body once", async () => {
    ...
    await expect((await app.fetch(new Request("http://example.test/", {
      ...
      headers: { "content-type": "application/json" },
      body: "{\"value\":1}",
    }))).json()).resolves.toMatchObject({ identical: true, first: { value: 1 }, bodyUsed: true });
  });

  it("should safely bind keys inherited by ordinary object prototypes", async () => {
    const response = await bindingApp().fetch(new Request(
      "http://example.test/items/1?__proto__=safe&constructor=also-safe",
      { method: "POST" },
    ));
    const result = await response.json() as Record<string, unknown>;
